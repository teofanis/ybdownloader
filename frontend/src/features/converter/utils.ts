...
export function getPresetName(preset: ConversionPreset, t: (key: string) => string): string {
  const key = `converter.presetNames.${preset.id}`;
  const translated = t(key);
  return translated === key ? preset.name : translated;
}

export function getPresetDescription(preset: ConversionPreset, t: (key: string) => string): string {
