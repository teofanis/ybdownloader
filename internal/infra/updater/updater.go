...
func (u *Updater) getLatestRelease(ctx context.Context) (*GitHubRelease, error) {
	...
	resp, err := u.httpClient.Do(req) //nolint:gosec // G704: URL is hardcoded GitHub API endpoint
	if err != nil {
		return nil, err
	}
...
`, u.downloadPath, currentAppImage, currentAppImage, currentAppImage, u.downloadPath)
	...
	// #nosec G306 -- Script needs to be executable to run the update
	if err := os.WriteFile(scriptPath, []byte(script), 0755); err != nil { //nolint:gosec // G703: script path is in app's temp directory
		return err
	}

	// #nosec G204 -- scriptPath is constructed in our temp directory
	cmd := exec.Command("bash", scriptPath) //nolint:gosec // G702: script is generated by the app, not from user input
	return cmd.Start()
}
